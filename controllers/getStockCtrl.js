...
const saveStockToUser = (req, res, objectGiven) =>
{
	singleUser.findById(req.session.passport.user, (err, foundUser) =>
	{
		...
		if(!stockExists)
		{
			...
			singleUser.findByIdAndUpdate(req.session.passport.user, {$push: {"stocks": { "symbol" : objectGiven.symbol, "stockId" :objectGiven._id, "quantity" : parseInt(req.params.quantity)}}}, (err, result) =>
			{
				if (err) throw err;
